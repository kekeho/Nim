...
.. raw:: html
  <blockquote><p>
  "人間は、目を持つ生き物である。 -- 私は、美しいものを望む。 (上記訳)"
  </p></blockquote>


このドキュメントは、 *Nim言語* のチュートリアルです。
このチュートリアルは、あなたがNimの変数、型、文法の基本的なことについて親しみを持ってもらうために
書かれています。しかしとても基本的なことに留めているので、Nimについてより詳しく知りたければ `manual <manual.html>`_
も見てみてください。より多くの例や、専門的な内容が盛り込まれています。
このチュートリアル中の全てのコードと、他のNimドキュメントのコードは `Nim style guide <nep1.html>`_
に沿って書かれています。あなたも、これにならってコードを書くようにしましょう。


はじめてのプログラミング
====================

それでは、ちょっと変わった"hello world"プログラムを書いてNimの旅へと出かけましょう:
...
このコードを"greetings.nim"というファイル名で保存してください(訳者注: 文字コードはUTF-8で)。
さあ、コンパイルしてプログラムを走らせてみましょう!::

  nim compile --run greetings.nim

``--run`` `スイッチ <nimc.html#compiler-usage-command-line-switches>`_ をつけることで、コンパイル後に自動で実行されます。
また、ファイル名の後に続けてプログラムに引数を渡すことができます::

  nim compile --run greetings.nim arg1 arg2

頻繁に使うコマンドとスイッチには略語が提供されています。なので以下のようにコンパイル
&実行することもできます::

  nim c -r greetings.nim

リリースバージョンとしてコンパイルするには以下のコマンドを実行します::

  nim c -d:release greetings.nim

デフォルトのNimコンパイラは、あなたがデバッグしやすいよう大量ののランタイムチェックを生成します。
 ``-d:release`` を付けることによって、
`それらのチェックをなくし、最適化を有効にします <nimc.html#compiler-usage-compile-time-symbols>`_ 。

(訳者注: Nimの美しい文法のおかげで)プログラムがなにをしているかは明らかでしょうが、
一応説明しておきましょう。
まずプログラムが開始されると、インデントされていない部分から実行されます。インデントは、Nimにおいて
コードをグループ化する方法です。インデントはスペースを用いたソフトタブで行うべきで、タブによるインデント
はよくありません。

文字列は、ダブルクオーテーション(")で囲われます。 The ``var`` statement declares
...
Nimのコードを構成する要素について、もっと見ていきましょう。
他のプログラミング言語のように、Nimも文字列リテラル、識別子、キーワード、コメント、演算子、また様々な
記号を持っています。
...
文字列リテラルは、ダブルクオーテーションで囲われます。 そして、1文字の場合はシングルクォートで囲みます。
エスケープシーケンスは、他の言語同様 ``\`` のあとに続けます。 ``\n`` は改行、 ``\t`` は
タブを表します。そして以下の例は、 *raw* 文字列を表します:

.. code-block:: Nim
  r"C:\program files\nim"

raw文字列は、バックスラッシュ(\)がエスケープ文字として認識されません。

3つ目、そして最後の文字列を表す方法は *long文字列* です.
次のように、3つのダブルクオーテーションで囲います。: ``""" ... """``
これは複数行に渡って書くことができます。 
そして、バックスラッシュシーケンス( ``\`` ) はエスケープシーケンスとして扱われません。
例えばHTMLなどをlong文字列として書くと便利ですね。


コメント
--------

コメントは、文字列や文字の中以外ならどこにでも、 ``#`` を使って書くことができます。
ドキュメンテーションコメントは、 ``##`` で記述できます。

.. code-block:: nim
    :test: "nim c $1"
  # コメント

  var myVariable: int ## ドキュメンテーションコメント


ドキュメンテーションコメントはトークンとなっています。
ドキュメンテーションコメントは構文ツリーに属しているので、特定の場所でのみ記述することができます。
これにより、各種ドキュメンテーションツールで変数などに対するコメントを表示することが可能になります。

複数行に渡るコメントは  ``#[`` で始まり、 ``]#`` で閉じることができます。
また、複数行コメントはネスト、つまり入れ子状に記述することが可能です。

.. code-block:: nim
    :test: "nim c $1"
  #[
  Nimコードのテキストにコメントを付けることができます。
  コメントの中では、インデントの制限がないので自由にコメントアウトできます。
        yes("くだらない質問をしてもいいかい?")
    #[
       Note: ほら、ネストできてるでしょ!!
    ]#
...
さらに、 `discard文 <#procedures-discard-statement>`_ を使うことで *long文字列*
でブロックコメントを書くこともできます。

.. code-block:: nim
    :test: "nim c $1"
  discard """ Yコメントの中では、インデントの制限がないので自由にコメントアウトできます。
        yes("くだらない質問をしてもいいかい?") """
...
特別な工夫として、次のように可読性を上げるためにアンダーラインを用いることができます: ``1_000_000`` (one million).
...
次に示すように ``var`` キーワードの後にインデントを入れることで、ひとまとまりに変数を宣言することができます。
